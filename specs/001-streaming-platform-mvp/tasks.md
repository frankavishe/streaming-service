...
- [ ] T002 [P] Initialize the NestJS backend project in `backend/` (NestJS 10, TypeScript 5.x,
      Node 20 LTS) with `backend/package.json`, `backend/tsconfig.json`, `backend/nest-cli.json`
- [ ] T003 [P] Initialize the Next.js frontend project in `frontend/` (Next.js 14+ App Router,
      TypeScript 5.x) with `frontend/package.json`, `frontend/tsconfig.json`
- [ ] T004 [P] Configure ESLint + Prettier for the backend in `backend/.eslintrc.cjs` and
      `backend/.prettierrc`
- [ ] T005 [P] Configure ESLint + Prettier for the frontend in `frontend/.eslintrc.cjs` and
      `frontend/.prettierrc`
- [ ] T006 [P] Configure Jest + Supertest for backend tests in `backend/jest.config.ts`
- [ ] T007 [P] Configure Jest + React Testing Library for frontend tests in
      `frontend/jest.config.ts`
- [ ] T008 Create a `docker-compose.yml` skeleton at the repository root defining `postgres`,
      `redis`, and `minio` services (Constitution Principle III)
- [ ] T009 Create `.env.example` at the repository root documenting DB credentials, JWT
...
- [ ] T010 Define `User` (`id` UUID PK; `email` string, unique; `passwordHash` string, never
...
- [ ] T011 Add `Title` (`id` UUID PK; `type` enum `MOVIE`|`SERIES`; `name` string; `description`
...
- [ ] T012 Add `MediaAsset` (`id` UUID PK; `ownerType` enum `TITLE`|`EPISODE`; `ownerId` UUID;
...
- [ ] T013 Add `Subscription` (`id` UUID PK; `userId` FK→User; `status` enum `ACTIVE`|`EXPIRED`;
...
- [ ] T014 Generate and apply the initial Prisma migration (`prisma migrate dev --name init`)
      producing `backend/prisma/migrations/`
- [ ] T015 [P] Create a seed script in `backend/prisma/seed.ts` that creates one default `ADMIN`
      user and two `Genre` rows, matching quickstart.md's documented first-run seed behavior
- [ ] T016 [P] Implement `PrismaService`/`PrismaModule` in `backend/src/prisma/prisma.service.ts`
      and `backend/src/prisma/prisma.module.ts`
- [ ] T017 [P] Implement environment configuration loading in
...
- [ ] T018 [P] Implement a global exception filter and structured logger in
      `backend/src/common/filters/http-exception.filter.ts` and
      `backend/src/common/logger/logger.service.ts`
- [ ] T019 Bootstrap the Nest application in `backend/src/main.ts`: global `ValidationPipe`,
      CORS configuration, the exception filter from T018, and the `/api` route prefix
- [ ] T020 [P] Implement the object storage wrapper in
...
- [ ] T021 [P] Implement the BullMQ/Redis connection module in
      `backend/src/queue/queue.module.ts`, registering the transcode job queue
- [ ] T022 Implement JWT + Passport auth scaffolding in `backend/src/auth/jwt.strategy.ts` and
      `backend/src/auth/auth.module.ts`: short-lived access tokens (15 min) and rotating
      refresh tokens (30 days) issued as httpOnly cookies (Constitution Principle IV)
- [ ] T023 Implement `POST /api/auth/register` in `backend/src/auth/auth.controller.ts` and
...
- [ ] T024 Implement `POST /api/auth/login` in `backend/src/auth/auth.controller.ts`: `200` with
      `{ user, accessToken, refreshToken }`; `401` on invalid credentials
- [ ] T025 Implement `POST /api/auth/refresh` in `backend/src/auth/auth.controller.ts`: rotates
...
- [ ] T026 Implement `POST /api/auth/logout` in `backend/src/auth/auth.controller.ts`: revokes
      the current refresh token, `204`
- [ ] T027 Implement `GET /api/auth/me` in `backend/src/auth/auth.controller.ts`: `200` with
      `{ id, email, role, subscription: { status, expiresAt } | null }`; `401` if unauthenticated
- [ ] T028 [P] Implement `RolesGuard` in `backend/src/common/guards/roles.guard.ts`, restricting
      access to `role = ADMIN` (FR-019) — used by every `/api/admin/*` endpoint
- [ ] T029 [P] Implement `EntitlementGuard` in
...
- [ ] T030 [P] Unit tests for `EntitlementGuard` covering subscribed, unsubscribed, and
...
- [ ] T031 [P] Scaffold the frontend app shell in `frontend/src/app/layout.tsx`, an API client in
...
- [ ] T032 [P] Implement the shared video player component in
...
- [ ] T033 [P] [US1] Implement `CatalogService` in `backend/src/catalog/catalog.service.ts`:
      ...
      titles/episodes are ever returned (FR-001, FR-018)
- [ ] T034 [US1] Implement `GET /api/catalog/titles` in
...
- [ ] T035 [US1] Implement `GET /api/catalog/titles/:titleId` in
      ...
      `processingStatus = READY`; `404` if not found or not published (depends on T033)
- [ ] T036 [US1] Implement `GET /api/catalog/episodes/:episodeId` in
      `backend/src/catalog/catalog.controller.ts`, same episode shape as above (depends on T033)
- [ ] T037 [P] [US1] Implement `MediaService.getTrailerUrl` in
...
- [ ] T038 [US1] Implement `GET /api/media/:assetId/trailer-url` in
...
- [ ] T039 [US1] Implement `GET /api/media/:assetId/playback-url` in
...
- [ ] T040 [P] [US1] Build the catalog browse page in `frontend/src/app/(public)/page.tsx`
      listing published titles with poster art and basic info
- [ ] T041 [P] [US1] Build the title detail page in
...
- [ ] T042 [US1] Wire the "play full" action on the title/episode detail pages to call
...
- [ ] T043 [US1] Add loading, error, and empty states to the catalog listing and trailer
...
- [ ] T044 [US2] Define the `PaymentProvider` interface (`initiate`, `verify`, `handleWebhook`)
      in `backend/src/payments/providers/provider.interface.ts`
- [ ] T045 [P] [US2] Implement `MpesaProvider` in
...
- [ ] T046 [P] [US2] Implement `BankGatewayProvider` in
      ...
      (depends on T044)
- [ ] T047 [US2] Implement `TransactionsService` in
...
- [ ] T048 [US2] Implement `SubscriptionsService.activateFromTransaction` in
...
- [ ] T049 [US2] Implement subscription-status derivation in
...
- [ ] T050 [P] [US2] Unit tests for `SubscriptionsService` covering activation, read-time
...
- [ ] T051 [US2] Implement `POST /api/payments/mpesa/initiate` in
...
- [ ] T052 [US2] Implement `GET /api/payments/:transactionId/status` in
      `backend/src/payments/payments.controller.ts` returning
      `{ transactionId, status, resolvedAt }` for frontend polling (FR-011)
- [ ] T053 [US2] Implement `POST /api/payments/mpesa/callback` in
...
- [ ] T054 [P] [US2] Integration tests for the M-Pesa webhook handler covering signature
      ...
      validates SC-007)
- [ ] T055 [US2] Implement `POST /api/payments/bank-gateway/initiate` in
...
- [ ] T056 [US2] Implement `POST /api/payments/bank-gateway/webhook` in
...
- [ ] T057 [P] [US2] Integration tests for the bank-gateway webhook handler covering signature
...
- [ ] T058 [US2] Implement a background timeout sweep for M-Pesa STK pushes that never resolve:
...
- [ ] T059 [P] [US2] Build the subscribe/checkout page in
      `frontend/src/app/(account)/subscribe/page.tsx` letting the user choose M-Pesa or bank
      payment
- [ ] T060 [US2] Build the M-Pesa flow UI in `frontend/src/app/(account)/subscribe/page.tsx`:
      ...
      T059)
- [ ] T061 [US2] Build the bank-gateway flow UI in
...
- [ ] T062 [US2] After a successful subscribe, refresh entitlement state (re-fetch
...
- [ ] T066 [P] [US4] Implement `AdminTitlesService` in
...
- [ ] T067 [US4] Implement `POST /api/admin/titles` and `PATCH /api/admin/titles/:titleId` in
...
- [ ] T068 [US4] Implement `POST /api/admin/titles/:titleId/seasons` (`422` unless the title's
      `type = SERIES`) and `POST /api/admin/seasons/:seasonId/episodes` in
      `backend/src/catalog/admin/admin.controller.ts`
- [ ] T069 [US4] Implement `PATCH /api/admin/episodes/:episodeId` in
      `backend/src/catalog/admin/admin.controller.ts` for metadata and/or `published` flag edits
- [ ] T070 [US4] Implement `POST /api/admin/media-assets` in
      `backend/src/media/admin/admin-media.controller.ts`: registers a `MediaAsset`
      (`ownerType`, `ownerId`, `kind`) and returns a signed PUT `uploadUrl` via the storage
      wrapper (T020); the unique `(ownerType, ownerId, kind)` constraint from T012 enforces at
      most one `TRAILER` and one `FULL` asset per owner
- [ ] T071 [US4] Implement `POST /api/admin/media-assets/:assetId/complete-upload` in
      `backend/src/media/admin/admin-media.controller.ts`: enqueues a transcode BullMQ job
      (T021) for `kind = FULL` assets and sets `processingStatus = PENDING`; `202` with
      `{ assetId, processingStatus: "PENDING" }` (FR-016, FR-017)
- [ ] T072 [US4] Implement `GET /api/admin/media-assets/:assetId` in
      `backend/src/media/admin/admin-media.controller.ts` returning
      `{ assetId, processingStatus, failureReason? }` for polling (FR-018; Edge Case: transcode
      failure surfaced to the admin)
- [ ] T073 [US4] Implement the transcode processor in
...
- [ ] T078 [US4] Wire the "publish" toggle in the admin title UI to
...
- [ ] T079 [P] Add a MinIO bucket bootstrap script in `docker/minio/` creating the buckets used
      by `storage.service.ts` (T020) on first `docker compose up`
- [ ] T080 Finalize `docker-compose.yml` wiring all six services (`postgres`, `redis`, `minio`,
...
- [ ] T081 [P] Add rate limiting to the auth and payment-initiation endpoints in
      `backend/src/main.ts` / the relevant controllers (Constitution Principle IV)
- [ ] T082 [P] Enforce secure, HTTPS-only cookie flags for the refresh-token cookie when
      `NODE_ENV=production` (Constitution Principle IV)
- [ ] T083 [P] Write the root `README.md` documenting `docker compose up --build` bring-up and
...
- [ ] T085 [P] Add Playwright end-to-end coverage for the four user-story flows in
